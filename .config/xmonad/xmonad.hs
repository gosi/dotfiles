...
boxLeftIcon    = "/home/jwm224/.icons/xbm_icons/subtle/boxleft.xbm"  --left icon of dzen boxes
boxLeftIcon2   = "/home/jwm224/.icons/xbm_icons/subtle/boxleft2.xbm" --left icon2 of dzen boxes
boxRightIcon   = "/home/jwm224/.icons/xbm_icons/subtle/boxright.xbm" --right icon of dzen boxes
...
myXPConfig = defaultXPConfig
	{ font              = dzenFont
	, bgColor           = "#cc6600"   --Brown was colorBlack
	, fgColor           = colorWhite
	, bgHLight          = colorBlue
	, fgHLight          = "#cccccc"  --Lt.Gray  was colorBlack
	, borderColor       = "#999933"   --colorGrayAlt
	, promptBorderWidth = 1
	, height            = panelHeight
	, position          = Top
	, historySize       = 100
...
calendarCA :: CA
calendarCA = CA
	{ leftClickCA   = "/home/jwm224/dzencal.sh"
	, middleClickCA = "/home/jwm224/dzencal.sh"
	, rightClickCA  = "/home/jwm224/bin/dzencal.sh"
	, wheelUpCA     = "/home/jwm224/bin/dzencal.sh"
	, wheelDownCA   = "/home/jwm224/bin/dzencal.sh"
	}
...
myStartupHook =
	...
	(spawn "/home/jwm224/.xmonad/apps/haskell-cpu-usage.out 5") <+>
...
myLayoutHook =
	...
	allLayouts where
		...
		chatLayouts = myToggleL (withIM (0.2) (Title "Blood in, Blood out") myMosA) myChatName                                   --workspace 5 layouts
...
manageWindows = composeAll . concat $
	...
	, [ className =? c --> doShift (myWorkspaces !! 3) | c <- myWebS    ]
	, [ className =? c --> doShift (myWorkspaces !! 9) | c <- myCodeS   ]
	, [ className =? c --> doShift (myWorkspaces !! 1) | c <- myGfxS    ]
	, [ className =? c --> doShift (myWorkspaces !! 4) | c <- myChatS   ]
	, [ className =? c --> doShift (myWorkspaces !! 0) | c <- myAlt3S   ]
	, [ className =? c --> doShift (myWorkspaces !! 2) | c <- myGfxS    ]
	, [ className =? c --> doShift (myWorkspaces !! 7) | c <- myAlt3S   ]
	, [ className =? c --> doShift (myWorkspaces !! 1) | c <- myGfxS    ]
	, [ className =? c --> doShift (myWorkspaces !! 8) | c <- myAlt3S   ]
	...
	, [ isFullscreen   --> doFullFloat ]
		...
		myWebS    = ["Chromium", "Firefox", "Luakit"]
		myCodeS   = ["Vim"] 
		myChatS   = ["ranger", "weechat"]
		myGfxS    = ["Gimp", "gimp", "GIMP"]
		myAlt3S   = ["rTorrent"]
		myFloatCC = ["MPlayer", "mplayer", "Vlc", "vlc", "Gimp", "gimp"
		            , "MuPDF", "mupdf", "Zanthura", "zanthura", "PavuControl", "pavucontrol"
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
	...
	, ((modMask,               xK_o ), windowMenu)                            --Toggle minimaize/ maximize a Window
    ...
    , ((modMask,               xK_z),     toggleWS), ((modMask .|. mod1Mask, xK_Down), withFocused (keysMoveWindow (0,30)))  -- move floated window 10 pixels down
              -- Minimize a window
	...
	, ((modMask, xK_m), spawn "termite")                                                                                   --Launches Termite Terminal
	, ((modMask, xK_u), spawn "urxvt -pe tabbed")                                                                          --Launches Urxvt w/ tabs
	, ((modMask, xK_i), spawn "xterm")                                                                                     --Launches Xterm Terminal
	, ((modMask, xK_b), spawn "firefox")                                                                                   --Launches firefox
	, ((modMask .|. shiftMask, xK_b), spawn "chromium")                                                                    --Launches chromium
	, ((modMask, xK_s), spawn "thunar")                                                                                    --Launches thunar
	, ((modMask, xK_y), spawn "xmobar")                                                                                    --Launches xmobar
	, ((modMask .|. shiftMask,  xK_y), spawn "pkill xmobar")                                                               -- kills xmobar
	--, ((0, 0x1008ffa9), spawn "/home/jwm224/bin/touchpadtoggle.sh")                                                      --Toggle touchpad (xmodmap -pk | grep -i toggle)
 ...
 --   , ((0,                          0x1008ff16  ), spawn "rhythmbox-client --previous")
	, ((modMask, xK_F5), spawn "amixer set Master toggle")                                                          --Mute/unmute volume
	, ((modMask, xK_F6), spawn "amixer set Master 2%-")                                                             --Lower volume
	, ((modMask, xK_F7), spawn "amixer set Master 2%+")                                                             --Raise volume
	--, ((mod1Mask, xK_down), spawn "XF86AudioLowerVolume") 
	--, ((mod1Mask, xK_Down), spawn "XF86AudioRaiseVolume")
	...
	, ((0, xF86XK_AudioPrev), flashText myTextConfig 1 " Previous Song " >> spawn "/usr/bin/ncmpcpp prev")                --Prev song/ncmpcpp
	, ((mod1Mask, xK_Left), flashText myTextConfig 1 " Previous Song " >> spawn "/usr/bin/ncmpcpp prev")
	, ((0, xF86XK_AudioPlay), flashText myTextConfig 1 " Song Toggled " >> spawn "/usr/bin/ncmpcpp toggle")               --Toggle song/ncmpcpp
	, ((mod1Mask .|. controlMask, xK_Down), flashText myTextConfig 1 " Song Toggled " >> spawn "/usr/bin/ncmpcpp toggle")
	, ((0, xF86XK_AudioStop), flashText myTextConfig 1 " Song Stopped " >> spawn "/usr/bin/ncmpcpp stop")                 --Stop song/ncmpcpp
	...
	, ((0, xF86XK_ScreenSaver), spawn "/usr/bin/xscreensaver")                                                --Lock screen
	, ((0, 0xff14), spawn "/usr/bin/xlock")
	, ((0, xK_Print), spawn "/usr/bin/scrot '%Y-%m-%d_$wx$h.png'" >> flashText myTextConfig 1 " Screenshot Saved ")       --Take a screenshot
	--, ((modMask , xK_s), spawn "/home/jwm224/bin/turnoffscreen.sh")                                                       --Turn off screen
...
cpuUsage path v c = fileToLogger format "4" path where
